...
function Dapp() {
  ...
  //Event créer des que le compte connecter a créer une action lis l'event---------------------
  useEffect(() => {
    if (simpleStorage) {
      const cb = (account, str) => {
        setValue(str);
          toast({
            title: "You made a transatcion",
            description: `${account} set storage with value: ${str}`,
            status: "warning",
            position: "top-right",
            duration: 9000,
      ...
      const allFilter = simpleStorage.filters.DataSet(web3State.account);
      simpleStorage.on(allFilter, cb);
      return () => {
        simpleStorage.off(allFilter, cb);
      };
    }
  }, [simpleStorage, web3State.account, toast]);
// fin de la lecture de l'event all -----------------------------------------------------------
